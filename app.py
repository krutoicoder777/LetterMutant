...
def is_valid_word(word):
    ...
    # Получаем нормальную форму (начальную)
    normal_form = parsed[0].normal_form
    
    # Проверяем, что нормальная форма существует и имеет нормальную длину
    if len(normal_form) < 3:
        logger.info(f"❌ Слово '{word}' отклонено: нормальная форма '{normal_form}' слишком короткая")
        return False
    
    # Дополнительная проверка: слово должно иметь часть речи (сущ, прил, глагол и т.д.)
    tag = parsed[0].tag
...
def handle_login(data):
    ...
    emit('login_success', {
        ...
        'rank': stats['rank'] if stats else 'Новичок'
    })
...
def handle_submit_word(data):
    ...
    # Проверка по словарю через pymorphy3
